...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  env.each do |env|
    config.vm.define env['name'] do |srv|
      ...
      srv.vm.network 'private_network', ip: env['ipaddress1']
      srv.vm.network 'private_network', ip: env['ipaddress2']
      ...
      #srv.vm.provision 'shell' do |s|
       # s.inline = "sudo apt update && sudo apt install -y curl && sudo apt install -y python3-distutils && curl -s https://bootstrap.pypa.io/get-pip.py | python3"
      #end
